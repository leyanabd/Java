...
public class Quaternion {
	...
	public Quaternion multiply(final Quaternion that) {
		...
		// Coding the formula: s = s1 * s2 - v1.v2;
		final float newS = this.s*that.s - this.v[0]*that.v[0] - this.v[1]*that.v[1] - this.v[2]*that.v[2]; 
		
		//coding the formula v = s1v2 + s2v1 + v1xv2
		
		float i = (this.s * that.v[0]) + (that.s * this.v[0]) + (this.v[1] * that.v[2] - this.v[2] * that.v[1]); 
		float j = (this.s * that.v[1]) + (that.s * this.v[1]) + (this.v[2] * that.v[0] - this.v[0] * that.v[2]); 
		float k = (this.s * that.v[2]) + (that.s * this.v[2]) + (this.v[0] * that.v[1] - this.v[1] * that.v[0]); 
	...
	private float norm() {
		 ...
		 * ************* Implement Your Code Here *************
		 */
		return (float) Math.sqrt(this.s*this.s + this.v[0]*this.v[0] + this.v[1]*this.v[1] + this.v[2]*this.v[2]);
	}
